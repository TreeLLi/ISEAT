...
def validate(loader, model, criterion, args):
    ...
    for i, (aug, targets) in enumerate(loader, 1):
        aug = aug.to(args.device, non_blocking=True).requires_grad_(True)
        ...
        if args.eval_iter > 0:
            adv, _ = perturb(aug, targets, model, criterion, eps, eps, ig=ig)
            fgsm.update(accuracy(model(adv), targets)[0][0], bs)
            
            adv, _ = pgd(aug, targets, model, criterion, eps, eps_step, args.eval_iter, ig=ig)
