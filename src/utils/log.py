...
RECORD = Dict({'model' : ['arch', 'checkpoint', 'pretrained', 'freeze_ft', 'width', 'depth', 'activation'],
          'dataset' : ['dataset', 'resize'],
          'training' : ['criterion', 'optim', 'lr', 'batch_size', 'awp_gamma', 'lam',
                        'augment', 'compose', 'rst', 'annealing', 'momentum', 'weight_decay', 'swa']})
...
class Logger:
    ...
    def new(self, info):
        ...
        if info.advt:
            RECORD.training += ['eps', 'eps_step', 'max_iter', 'random_init', 'warm_start']
